...
class SphereParticle extends SendableParticle {
    ...
    public function __construct(private float $size, private float $space, private float $maxYaw = 360, private float $minPitch = -90, private float $maxPitch = 90){
    }

    public function draw(Position $pos): array{
        $positions = [];
        for($yaw = 0; $yaw < $this->maxYaw; $yaw += $this->space){
            for($pitch = $this->minPitch; $pitch < $this->maxPitch; $pitch += $this->space){
