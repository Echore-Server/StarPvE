		...
		if ($adapter instanceof JobConfigAdapter){
			...
			if ($player instanceof Player){
				...
				if ($job !== null){
					...
					$this->previous[spl_object_hash($player)] = $jobInstance->getIdentityGroup()->getActive($player);
				}
			}
		}
	}
		...
		if ($adapter instanceof JobConfigAdapter){
			...
			if ($player instanceof Player){
				...
				if ($job !== null){
					...
					$newIdentity = array_diff($jobInstance->getIdentityGroup()->getActive($player), $this->previous[spl_object_hash($player)] ?? []);
					foreach($newIdentity as $identity){
						$player->sendMessage("§d> §d{$identity->getName()} §7がアンロックされました！");
					}
