...
class DeathPulseSkill extends Skill {
	...
	public function getDescription(): string{
		...
		return
sprintf('§b発動時:§f 自分から %1$s の地点にパルスを発生させ、§c2.5秒§f かけて §c0m§f の地点まで移動する。
移動している間敵がパルスに触れた場合、その敵を %2$s 間動けなくする。
パルスが移動し終えたら、
自分から %1$s 以内にいる敵全てに敵の最大体力 %3$s 分のダメージを与える攻撃を %4$s、%5$s ごとに行う。', $area, $duration, $damagePer, $amount, $period);
	...
	protected function onActivate(): ActionResult{
		...
		TaskUtil::repeatingClosureLimit(function () use($preparePeriod, $prepareLimit, $std, $step){
			...
			$par->sendToPlayers($this->player->getWorld()->getPlayers(), $pos, "minecraft:obsidian_glow_dust_particle");
		...
		TaskUtil::delayed(new ClosureTask(function(){
			TaskUtil::repeatingClosureLimit(function(){
				...
				$pos = VectorUtil::keepAdd($this->player->getPosition(), 0, 0.9, 0);
				$par->sendToPlayers($this->player->getWorld()->getPlayers(), $pos, "minecraft:sparkler_emitter");
				
				$effect = (new SaturatedLineworkEffect($area, 3, 0, 12, 360, 0, 0));
				$effect->sendToPlayers($this->player->getWorld()->getPlayers(), $pos, "minecraft:obsidian_glow_dust_particle");
			...
			}, (30 - (integer) min(29, $this->speed->get())), (integer)$this->amount->get());
		}), ($preparePeriod * $prepareLimit));
